...
const InvoicePage = ({ history }) => {
    ...
    const handleChange = (event) => {
        ...
        console.log("whats hapened", event.target.value);
        setInvoice({ ...invoice,sentAt: new Date(), chron:2,  [event.target.name]: event.target.value });
    };
    ...
    const handleSubmit = async event => {
      ...
      await axios.post("http://localhost:89/api/invoices",
        {
            ...invoice,status:"SENT", customer: `/api/customers/${invoice.customer}`
        }
    //    ( {
    //         "amount": "500",
    //         "status": "SENT",
